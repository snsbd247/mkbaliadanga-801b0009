...
BEGIN
  ...
  FOR r IN
    ...
    WHERE c.table_schema = 'public'
      AND c.column_name = 'farmer_id'
      AND c.table_name <> 'farmers'
  ...
  FOR r IN
    ...
    JOIN information_schema.tables t
     ...
     AND t.table_name = c.table_name
    WHERE c.table_schema = 'public'
      AND c.column_name IN ('owner_farmer_id', 'sharecropper_farmer_id', 'source_farmer_id', 'recipient_farmer_id', 'cultivator_farmer_id')
      AND t.table_type = 'BASE TABLE'
  LOOP
    EXECUTE format(
