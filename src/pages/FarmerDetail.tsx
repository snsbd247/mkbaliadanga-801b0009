...
export default function FarmerDetail() {
  ...
  async function loadAll() {
    const [f, l, s, ln, ir, sh, pm] = await Promise.all([
      db.from("farmers").select("*, offices(name), divisions(name,name_bn), districts(name,name_bn), upazilas(name,name_bn), unions(name,name_bn), wards(name,name_bn), villages(name,name_bn), mouzas(name,name_bn)").eq("id", id!).maybeSingle(),
