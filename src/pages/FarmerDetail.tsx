...
export default function FarmerDetail() {
  ...
  async function reprintReceipt(p: any, copy: import("@/lib/bnReceipts").ReceiptCopy = "both") {
    ...
    if (kind === "irrigation") {
      ...
      irrigationExtras = {
        ...
        // Legacy fallback only: do not print the farmer's remaining ledger due
        // as this receipt's বকেয়া amount.
        total_outstanding: 0,
  ...
  async function deletePayment(p: any) {
    ...
    // Atomic server-side delete: removes irrigation links + allocations and the
    // payment in one transaction. Removing each link row fires the invoice recalc
    // trigger, restoring due_amount and invoice_status. FK cascades cover any
    // stragglers so nothing is left orphaned.
    const { error } = await (supabase as any).rpc("delete_payment_cascade", { _payment_id: p.id });
    if (error) return toast.error(error.message);
    toast.success(t("pgDeleted" as any)); loadAll();
  }
