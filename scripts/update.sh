...
# Remove generated/untracked files that commonly block a merge/pull. These are
# regenerated by composer/artisan below, so deleting them is always safe and
# prevents "untracked working tree files would be overwritten by merge" aborts.
