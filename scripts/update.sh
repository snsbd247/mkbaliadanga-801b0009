...
# Fetch FIRST (with retries) so a transient GitHub network hiccup fails the
# deploy cleanly WITHOUT having already deleted composer.lock / caches. Only
# once we have the new refs do we clean generated files and reset.
fetch_with_retry() {
...
# Now that new refs are local, remove generated/untracked files that commonly
# block a merge/reset. These are regenerated by composer/artisan below, so
...
# NOTE: `npm audit fix --force` is intentionally NOT run — it installs breaking
# SemVer-major upgrades (e.g. Vite 5 → 8) that break `npm run build`.
